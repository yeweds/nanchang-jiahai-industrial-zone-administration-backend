...
class NewsAction extends GlobalAction{
    ...
    public function index()
    {
        ...
        if(isset($_GET['ispublish'])){
            $where['h_news.ispublish'] = $_GET['ispublish'];
        }
        //所有新闻分类
        // dump($where);exit;
        ...
        $table2 = $fix."class"; 
        $table3 = $fix."user"; 
        $count= $NewsModel->where($where)->join("$table2 on $table.class_id=$table2.id")->count();
        // dump($count);exit;
        import("ORG.Util.Page"); //导入分页类 
        $listRows = 20;
        $p = new Page($count,$listRows); 
        $list = $NewsModel->where($where)->join("$table2 on $table.class_id=$table2.id")->join("$table3 on $table.user_id=$table3.id")->field("$table.* ,$table2.name,$table3.username")->limit($p->firstRow.','.$p->listRows)
              ->order("$table.add_time desc,$table.id desc")->select(); 
        // echo $NewsModel->getLastSql();exit;
        // dump($list);exit;
        $page=$p->show();
        if($list){
    ...
    public function edit()
    {
        ...
        //当前已关联---start
        $map_mp['news_id'] = $id;
        $news_mapping_list = M('News_mapping')->where($map_mp)->select();
