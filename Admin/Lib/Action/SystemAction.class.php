...
class SystemAction extends GlobalAction{
   ...
   //dump($web_config);
	...
	public function update_cfg()
	{
		$data = $_POST;
		//填充为空的项目		
		if($data["cfg_sitename"]=='')$data["cfg_sitename"]='婚庆网' ;
		if($data["cfg_domain"]=='')$data["cfg_domain"]='http://www.0791hunqing.com' ;
		if($data["cfg_allow_reg"]=='')$data["cfg_allow_reg"]= 'true';
		if($data["cfg_title"]=='')$data["cfg_title"]='婚庆网' ;
		if($data["cfg_metakeyword"]=='')$data["cfg_metakeyword"]='婚庆,南昌婚庆';
 ...
 * 修改楼盘采集映射记录
 +----------------------------------------------------------
 */
	public function edit_id_mapping()
	{
		$Table = M("Id_mapping");
		$id = intval($_GET['id']);
		$map['id'] = $id;
 ...
 * 更新楼盘采集映射表
 +----------------------------------------------------------
 */
	public function update_id_mapping()
	{
		$Table = M("Id_mapping");
		$map['class_id'] = 7;  //楼盘
		$list  = M('Info')->field('info_id,title,range_id')->where($map)->findAll();
		foreach($list as $k=>$v){
			$data['info_id'] = $v['info_id'];
			$data['lpname']  = $v['title'];
			$data['range_id']= $v['range_id'];
			if($rs = $Table->where('info_id='.$v['info_id'] )->find()){
				$Table->where('info_id='.$v['info_id'] )->save($data);
			}else{
				$Table->add($data);
			}
		}
		...
		$this->assign('waitSecond',5);
        $this->assign('jumpUrl', __URL__."/id_mapping");
		$this->success($count.'条info_id记录更新成功！');
