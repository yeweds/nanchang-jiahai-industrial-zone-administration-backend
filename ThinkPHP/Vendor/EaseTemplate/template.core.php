...
class ETCore
{
    ...
    public $Hacker		= "<?php if (!defined('ET3!')) {die('You are Hacker!<br>Power by Ease Template!');}";
    ...
    public function lang(
        ...
        if (is_dir($this->LangDir)) {
            ...
            if ($this->LangData[$id]=='' && $this->Language=='default') {
                ...
                //语言文件过大时采取建立新文件
                if (strlen($docs)>400) {
                    $this->writer($this->LangDir.$this->Language.'.'.$id.'.php','<?php $etl = "'.$str.'";?>');
