...
/*  IP 地理位置查询类 熊彦
 +------------------------------------------------------------------------------
 * @author    colin hong <admin@qunfei.com>
 * @version   1.0
 +------------------------------------------------------------------------------
 */
class IpFind extends Think
{
private $filename;
public function __construct()
{
    $this->filename = "./Public/Data/qqwry.dat";
    //halt($this->filename);
}
...
//查找字符串
    public function  findstr($str, $substr)
    {
            $m = strlen($str);
            $n = strlen($substr );
            if ($m < $n) return false ;
