<?php
    ...
    //执行字符串中的变量
    public function eval_str($cityname,$str)
    {
        eval( "\$str = \"$str\";" );
    ...
    //获取区域名称
    public function getAreaName($cityid)
    {
        $condition['id']    =    $cityid;
    ...
    //UTF-8 转GB编码
    public function utf82gb($utfstr)
    {
 ...
 * @return string
 +----------------------------------------------------------
 */
    public function ZHsubstr($str, $start=0, $length, $charset="utf-8")
    {
    ...
    public function clearHtml($str)
    {
        ...
        //根据大类ID得到子类
    public function getSmallClass($cid)
    {
        $class = D('Class');
        $condition['pid']  =  $cid;
        $condition['ishide']  =  1;
...
//获得分类表大类id
    public function getBigclassid($class_id)
    {
        //$class_id :分类ID
         $dao  = D("Class");  //
    ...
    //获得分类表父类路径
    public function getParent_path($class_id)
    {
        //$class_id :分类ID
         $dao  = D("Class");  //
    ...
    for ($i=0;$i<$slen;$i++) {
        ...
        } elseif ( eregi("[a-z0-9]",$str[$i]) ) {	$restr .= $str[$i]; } else { $restr .= "-";  }
    }
...
//参数 act：1文字，2图片,默认为文字
    public function getLinks($act=1,$num=27)
    {
        $where['passed'] = 1;
        if ($act == 2) {
...
//得到用户信息
    public function getUserVO($user,$field='*')
    {
        if (is_numeric($user)) {
            $where['id'] = $user;
        } else {
...
/* 加密，可逆. 可接受任何字符*/
    public function encrypt($txt, $key = '100rNet')
    {
...
function out_str($str)
{
    ...
    //根据分类ID取得其内容模型
    public function getTemplet($class_id)
    {
        //$class_id :分类ID
         $dao  = D("Class");  //
         $vo = $dao->getById($class_id);
...
//---获取综合点评  --熊彦
    public function getLpdpXml($data)
    {
...
//---$status= success成功/error失败, info提示信息, data传递参数
    public function ajaxMsg($info='', $status='success', $title='', $data='')
    {
        if (empty($info)) {
            $info = '操作成功！';
        }
...
//随机生成用户名 --用于随机发点评
    ...
    public function my_addslashes($string)
    {
...
//换引号
    public function mystr_replace($str)
    {
        return str_replace('"',"'",$str);
    }

    //标题样式自定义
    public function title_style($str, $default="")
    {
        switch ($str) {
...
//获取新闻文章链接(统一普通链接和转向链接)
    public function get_arc_url($id, $redirecturl="")
    {
        //跳转网址
        if ($redirecturl != '') {
            $url = $redirecturl;
